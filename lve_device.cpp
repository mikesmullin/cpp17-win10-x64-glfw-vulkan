...
LveDevice::LveDevice(LveWindow &window) : window{window} {
  createInstance();       // initialize Vulkan lib its connection to our application
  setupDebugMessenger();  // enable error checking (disable in production!)
  createSurface();        // GLFW connection between window and Vulkan's output
  pickPhysicalDevice();   // choose one or more GPUs
  createLogicalDevice();  // enable the features of GPU we want to use
  createCommandPool();    // command buffer allocation for issuing instructions to
                          // GPU
}
